...
const ArtistCard: React.FC<ArtistCardProps> = ({ artist, index = 0 }) => {
  return (
    <div 
      className={`group relative h-[400px] md:h-[480px] overflow-hidden bg-surface border border-transparent hover:border-brand/40 hover:shadow-[0_0_40px_rgba(255,77,0,0.2)] transition-all duration-500 cursor-pointer ${index % 2 !== 0 ? 'md:translate-y-8' : ''}`}
    >
      <img 
        src={artist.imageUrl} 
        alt={artist.name} 
      ...
      <div className="absolute inset-0 p-6 flex flex-col justify-end bg-gradient-to-t from-canvas via-canvas/60 to-transparent opacity-90 group-hover:opacity-100 transition-opacity duration-500">
        <div className="transform translate-y-6 group-hover:translate-y-0 transition-transform duration-500 ease-out">
          <span className="inline-block px-3 py-1 border border-brand text-brand group-hover:bg-brand group-hover:text-canvas transition-colors duration-500 text-[10px] font-bold uppercase mb-4 shadow-[0_0_10px_rgba(255,77,0,0)] group-hover:shadow-[0_0_15px_rgba(255,77,0,0.4)]">
            {artist.genre}
          </span>
          
          <div className="flex items-center gap-4 mb-2">
            <h3 className="font-display text-3xl text-white group-hover:text-brand transition-colors duration-500">{artist.name}</h3>
          ...
          <div className="h-[1px] w-0 group-hover:w-12 bg-brand transition-all duration-500 delay-200 mb-3"></div>
